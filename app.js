...
function fFirma(val){
  var NORD=['','PRIMERO','SEGUNDO','TERCERO','CUARTO','QUINTO','SEXTO','SÉPTIMO','OCTAVO','NOVENO','DÉCIMO',
    'UNDÉCIMO','DUODÉCIMO','DÉCIMO TERCERO','DÉCIMO CUARTO','DÉCIMO QUINTO','DÉCIMO SEXTO','DÉCIMO SÉPTIMO','DÉCIMO OCTAVO','DÉCIMO NOVENO','VIGÉSIMO',
    'VIGÉSIMO PRIMERO','VIGÉSIMO SEGUNDO','VIGÉSIMO TERCERO','VIGÉSIMO CUARTO','VIGÉSIMO QUINTO','VIGÉSIMO SEXTO',
    'VIGÉSIMO SÉPTIMO','VIGÉSIMO OCTAVO','VIGÉSIMO NOVENO','TRIGÉSIMO','TRIGÉSIMO PRIMERO'];
  if(!val)return '_________ (XX) días del mes de ________________ del año 2.0XX';
  var d=new Date(val+'T00:00:00');
  var dia=d.getDate();
...
function initClauseEditor(){
  ...
  var NUMS=['PRIMERA','SEGUNDA','TERCERA','CUARTA','QUINTA','SEXTA','SÉPTIMA','OCTAVA','NOVENA','DÉCIMA','DÉCIMO PRIMERA','DÉCIMO SEGUNDA','DÉCIMO TERCERA','DÉCIMO CUARTA','DÉCIMO QUINTA','DÉCIMO SEXTA','DÉCIMO SÉPTIMA','DÉCIMO OCTAVA','DÉCIMO NOVENA','VIGÉSIMA','VIGÉSIMA PRIMERA','VIGÉSIMA SEGUNDA','VIGÉSIMA TERCERA','VIGÉSIMA CUARTA'];
  
  // Determine which clauses apply based on tipo
  var L=TIPO==='local', D=(TIPO==='depto'||TIPO==='estrenar'), C=TIPO==='country';
  var numCls = (D||L) ? 24 : (C ? 24 : 23);
  ...
  for(var i=0;i<numCls;i++){
    (function(nom){
      ...
      hdr.style.cssText = 'background:var(--bg);padding:7px 12px;font-size:11px;font-weight:700;color:var(--az);display:flex;align-items:center;justify-content:space-between;cursor:pointer';
      var sp = document.createElement('span'); sp.textContent = 'Cláusula '+nom;
      var badge = document.createElement('span');
      badge.id = 'ecl-badge-'+key;
      badge.style.cssText = 'font-size:10px;color:var(--g);font-weight:normal';
      badge.textContent = 'Sin cambios ▼';
      hdr.appendChild(sp); hdr.appendChild(badge);
      hdr.onclick = function(){ toggleECl(key); };
      
      var body = document.createElement('div');
      body.id = 'ecl-body-'+key;
      body.style.cssText = 'display:none;padding:8px';
      
      var ta = document.createElement('textarea');
      ta.id = 'ecl-ta-'+key;
      ta.placeholder = 'Texto de la cláusula '+nom+' (se genera automáticamente al descargar si lo dejás vacío)';
      ta.style.cssText = 'width:100%;min-height:80px;font-size:11px;font-family:inherit;padding:7px;border:1px solid var(--bd);border-radius:5px;resize:vertical';
      (function(n){ ta.oninput = function(){ onEClChange(n); }; })(nom);
...
function onEClChange(nom){
  ...
  if(!ta) return;
  var t = ta.value.trim();
  if(t) CLAUSE_EDITS[nom] = t;
  else delete CLAUSE_EDITS[nom];
  var badge = document.getElementById('ecl-badge-'+key);
...
function renderGars(){
  ...
  for(var i=0;i<GARS.length;i++){
    ...
    r3.innerHTML=
      ...
      '<option value="inmueble">Inmueble (matrícula)</option>'+
...
function gTxt(id){
  ...
  var desc=v(id+'-desc')||'(descripción del encabezado de la matrícula)';
  var org=v(id+'-org')||'XXX';
  var cui=v(id+'-cuit')||'XXXXXXXX';
  if(vt==='inmueble') return 'presenta como garantia su propiedad sobre un inmueble inscripto ante el Registro General de la Provincia a la Matrícula '+mat+' Capital (XX) descripto como: '+desc;
...
function setMoneda(btn){
  btn.closest('.gbtn').querySelectorAll('.gb').forEach(function(b){b.classList.remove('on');});
  btn.classList.add('on');
  MONEDA = btn.dataset.m;
  // Update alq placeholder
  var alqEl = document.getElementById('alq');
...
function calcP(){
  ...
  var n=parseFloat(s);
  if(!isNaN(n)&&n>0) document.getElementById('pag-m').value=(MONEDA==='dolares'?'USD ':'$')+(n*2).toLocaleString('es-AR');
...
function limpiar(){
  ...
  GEN={loc:'m',loc2:'m'}; GARS=[]; GC=0; LOCS=[]; LC=0; MONEDA='pesos';
  document.querySelectorAll('.gbtn .gb').forEach(function(b){b.classList.toggle('on',b.dataset.g==='m'||b.dataset.m==='pesos');});
  CLO={}; CLE={};
  hdn('pnl-cls',true);
...
function getRest(){
  var mp={'r-caldera':'caldera de calefacción y radiadores','r-calef':'calefactores','r-aire':'equipos de aire acondiciónado','r-pisc':'equipo de mantenimiento de piscina'};
  var r=[];
...
function numLetras(n){
  ...
  var cientos=n%1000;
  var res='';
  var u=['','UN','DOS','TRES','CUATRO','CINCO','SEIS','SIETE','OCHO','NUEVE','DIEZ','ONCE','DOCE','TRECE','CATORCE','QUINCE','DIECISEIS','DIECISIETE','DIECIOCHO','DIECINUEVE','VEINTE','VEINTIUN','VEINTIDOS','VEINTITRES','VEINTICUATRO','VEINTICINCO','VEINTISEIS','VEINTISIETE','VEINTIOCHO','VEINTINUEVE'];
  var dec=['','','VEINTE','TREINTA','CUARENTA','CINCUENTA','SESENTA','SETENTA','OCHENTA','NOVENTA'];
  var cen=['','CIEN','DOSCIENTOS','TRESCIENTOS','CUATROCIENTOS','QUINIENTOS','SEISCIENTOS','SETECIENTOS','OCHOCIENTOS','NOVECIENTOS'];
  function grupo(num){
    var s='';
  ...
  if(millones>0){
    res+=grupo(millones)+(millones===1?' MILLON ':' MILLONES ');
  }
  if(miles>0){
    if(miles===1) res+='MIL ';
    else res+=grupo(miles)+' MIL ';
  }
  if(cientos>0){
    res+=grupo(cientos);
  }
...
async function genWord(){
  try {
    ...
    var bajaTxt=bajaS
      ?'Con respecto a los servicios de luz y gas natural (si correspondiera), los mismos deberán ser dados de baja y abonados hasta la fecha de entrega del inmueble, debiendo entregar los comprobantes correspondientes en el domicilio de la empresa administradora.-'
      :'Con respecto a los servicios de luz y gas natural (si correspondiera), los mismos deberán ser abonados y no dados de baja, debiendo entregar los comprobantes originales pagados correspondientes en el domicilio de la empresa administradora.-';
    var lG=gT('loc'), l2G=gT('loc2');
    var alqN=parseFloat(v('alq').replace(/[.,]/g,''))||0;
    var pagMnum=alqN*2;
    var esDolar=(MONEDA==='dolares');
    var simb=esDolar?'USD ':'$';
    var pagM=alqN>0?simb+pagMnum.toLocaleString('es-AR'):'XXXXXXXXXXXX';
    var pagMletras=alqN>0?(esDolar?'DOLARES '+pagMnum.toLocaleString('es-AR'):numLetras(pagMnum)):'XXXXXXXXXXXX';
    ...
    var ni=0;
    var _lastClause='';
    function cl(){_lastClause=NUMS[ni++];return _lastClause+':';}
    function wPcl(txt,opts){
      var edit=getClauseEdit(_lastClause);
      return wP(edit||txt,opts);
    }
    var c='';
    ...
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('El Locatario, permitirá al Locador, o a quien lo represente la inspección de la propiedad cuando este lo estime necesario, y en caso de producirse algun desperfecto que afecte la estabilidad o seguridad de la propiedad, el Locatario esta obligado a comúnicarlo en forma fehaciente al Locador, dentro de las veinticuatro (24) horas de producido.'+(hasMasc?' Queda expresamente prohibido tener mascotas de cualquier raza y especie, siendo este, causal de rescisión del presente contrato debiendo abonar el locatario las penalidades previstas para los casos de rescisión anticipada.-':' '));
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('El Locatario no podrá efectuar en la propiedad ninguna innovación, alteración o mejora en la construcción sin el previo consentimiento por escrito del Locador'+(L?'; las que se hiciesen debidamente autorizadas quedarán en beneficio exclusivo del inmueble, sin cargo de indemnización.':'. Las que se hiciesen, debidamente autorizadas, quedarán en beneficio exclusivo del inmueble sin cargo de indemnización.')+' Las mejoras que se introduzcan deberán respetar todo lo atinente a disposiciónes municipales y provinciales vigentes, estando estrictamente prohibida la alteración funcional de la propiedad.-');
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('El Locatario se obliga a registrar de inmediato '+(L?'ante la empresa de electricidad y gas, los servicios de Luz y Gas (si correspondiere),':'ante las empresas de electricidad y gas (si correspondiere) los servicios correspondientes,')+' a su nombre y a dejarlos pagos hasta el dia en que haga devolución del inmueble, declarando conocer que no debe abonar factura alguna por servicios anteriores a la fecha de iniciacion del contrato. El Locatario se compromete a contratar '+(SE?'un seguro de incendio y responsabilidad civil sobre el inmueble locado':'un seguro de incendio, y responsabilidad civil hacia linderos sobre el inmueble locado')+' por el periodo que dure este contrato, endosando la poliza'+(SE?'':' de incendios')+' a favor del Locador, con actualización anual. El Locatario deberá entregar la poliza dentro de los 15 (quince) dias de firmado el contrato.-');
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('Quedan por cuenta y cargo exclusivo del Locatario y TAMBIEN FORMAN PARTE DEL PRECIO DE LA LOCACION el pago en término del servicio de obras sanitarias y/o agua domiciliaria (unidad de facturación número de cuenta '+(v('san')||'XXXX')+') y/o cualquier otro servicio y/o accesorio que recae sobre la unidad locada, como asi también el pago de la tasa municipal por alumbrado, barrido y limpieza (identificación catastral '+(v('catastro')||'XXX')+') y el pago del impuesto provincial (DGR número de cuenta '+(v('dgr')||'XXXXX')+'). Las obligaciónes descriptas serán abonadas por la empresa administradora quien posteriormente las cobrara al Locatario. Por lo tanto no se considerará cumplida la prestacion dineraria por parte del Locatario ni será obligación del Locador recibir el pago, si no se abonará conjuntamente con el alquiler los servicios, expensas y accesorios precedentemente mencionados, correspondientes al mes en curso. En el caso de mero atraso, los intereses, reajustes, punitorios y/o adicionales correrán por exclusiva cuenta del Locatario.-');
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('Se deja expresa constancia que el Locatario suscribe como garantia de la presente locación un pagaré por la suma de '+(esDolar?'Dólares Estadounidenses':'Pesos')+' '+pagMletras+' ('+pagM+'.-) con fecha de vencimiento el dia '+pagV+'. El mismo será devuelto al Locatario siempre que se hayan cancelado todas las obligaciónes emanadas del presente. En caso de producirse la rescisión anticipada del presente, el pagaré se utilizará a los mismos efectos descriptos precedentemente y no se aplicara a las indemnizaciónes que debiera abonar el Locatario por la resolución anticipada del contrato.-');
    if(D||L){
      c+=wP(cl(),{b:true,sb:200,sa:80});
      c+=wPcl('Son a cargo del Locatario y también FORMAN PARTE DEL PRECIO DE LA LOCACION, ademas de lo estipulado en la cláusula tercera, el pago de las Expensas Comúnes Ordinarias asignadas a la unidad locada, por el consorcio del que forma parte y conforme a las disposiciónes del Reglamento de Copropiedad. Los importes correspondientes a Expensas Extraordinarias serán soportados por el Locador, y en el caso que sean incluidos dentro del cupon de pago de las expensas comúnes mensuales, estos serán abonados por el Locatario y descontados del importe a abonar en concepto de canon locativo correspondiente al mismo periodo. Por lo tanto no se considerará pagado el alquiler mensual, ni será obligación del Locador recibir el pago del mismo, si no se abonan las Expensas Comúnes en la forma establecida por ser estos PARTE INTEGRANTE DEL PRECIO DEL ALQUILER. El Locatario deberá acatar todas las normas contenidas en el reglamento de copropiedad y reglamento interno del edificio, las que declara conocer y aceptar.'+(v('consorcio')?' ('+v('consorcio')+').-':'.-'));
    }
    c+=wP(cl(),{b:true,sb:200,sa:80});
    var moraT='';
    if(C) moraT='La falta de pago en término del alquiler, las Expensas o Gastos Comúnes o cualquier otra obligación a cargo del Locatario lo hará incurrir en mora de pleno derecho, sin necesidad de requerimiento previo de ninguna naturaleza y las sumas adeudadas devengarán un interes punitorio fijado a opcion del Locador, que en su caso establecerá según los siguientes sistemas: A) Un interes punitorio mensual igual a una vez y media (1,5) la tasa efectiva mensual vencida (T.E.M. 30 dias) que pública el Banco de la Nacion Argentina. B) El equivalente al cero con treinta por ciento (0,30%) diario del monto de las sumas adeudadas, computandose su cálculo desde el primer dia calendario del mes en que se produzca la mora. Ello también sin perjuicio de iniciar el Locador, las acciones judiciales por cobro de alquileres, desalojo o cualquiera que corresponda en su caso, pactandose que los alquileres, servicios, expensas y demas accesorios, podrán ser reclamados por el procedimiento ejecutivo. El Locador podrá negarse a percibir cualquiera de estas obligaciónes, sin el pago conjunto de la penalidad establecida.-';
    else if(D) moraT='La falta de pago en término del alquiler, las Expensas o Gastos Comúnes o cualquier otra obligación a cargo del Locatario lo hará incurrir en mora de pleno derecho, sin necesidad de requerimiento previo de ninguna naturaleza y las sumas adeudadas devengarán un interes punitorio fijado a opcion del Locador, que en su caso establecerá según los siguientes sistemas: A) Un interes punitorio mensual igual a una vez y media (1,5) la tasa para descuentos de documentos a treinta (30) dias de plazo que aplica el Banco de la Nacion Argentina. B) El equivalente al cero con treinta por ciento (0,30%) diario del monto de las sumas adeudadas, computandose su cálculo desde el primer dia calendario del mes en que se produzca la mora. Ello también sin perjuicio de iniciar el Locador, las acciones judiciales por cobro de alquileres, desalojo o cualquiera que corresponda en su caso, pactandose que los alquileres, servicios, expensas y demas accesorios, podrán ser reclamados por el procedimiento ejecutivo. El Locador podrá negarse a percibir cualquiera de estas obligaciónes, sin el pago conjunto de la penalidad establecida.-';
    else if(L) moraT='La falta de pago en término del alquiler o cualquier otra obligación a cargo del Locatario lo hará incurrir en mora de pleno derecho, sin necesidad de requerimiento previo de ninguna naturaleza y las sumas adeudadas devengarán un interes punitorio fijado a opcion del Locador, que en su caso establecerá según los siguientes sistemas: A) Un interes punitorio mensual igual a una vez y media (1,5) la tasa para descuentos de documentos a treinta (30) dias de plazo que aplica el Banco de la Nacion Argentina. B) El equivalente al cero con treinta por ciento (0,30%) diario del monto de las sumas adeudadas, computandose su cálculo desde el primer dia calendario del mes en que se produzca la mora. Ello también sin perjuicio de iniciar el Locador, las acciones judiciales por cobro de alquileres, desalojo o cualquiera que corresponda en su caso, pactandose que los alquileres, tributos, servicios, expensas y demas accesorios podrán ser reclamados por el procedimiento ejecutivo. El Locador podrá negarse a percibir cualquiera de estas obligaciónes, sin el pago conjunto de la penalidad establecida.-';
    else moraT='La falta de pago en término del alquiler o cualquier otra obligación a cargo del Locatario lo hará incurrir en mora de pleno derecho, sin necesidad de requerimiento previo de ninguna naturaleza y las sumas adeudadas devengarán un interes punitorio fijado a opcion del Locador, que en su caso establecerá según los siguientes sistemas: A) Un interes punitorio mensual igual a una vez y media (1,5) la tasa para descuentos de documentos a treinta (30) dias de plazo que aplica el Banco de la Nacion Argentina. B) El equivalente al cero con treinta por ciento (0,30%) diario del monto de las sumas adeudadas, computandose su cálculo desde el primer dia calendario del mes en que se produzca la mora. Ello también sin perjuicio de iniciar el Locador, las acciones judiciales por cobro de alquileres, desalojo o cualquiera que corresponda en su caso, pactandose que los alquileres, servicios, expensas y demas accesorios, podrán ser reclamados por el procedimiento ejecutivo. El Locador podrá negarse a percibir cualquiera de estas obligaciónes, sin el pago conjunto de la penalidad establecida.-';
    c+=wP(moraT);
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('Cualquier incumplimiento a lo estipulado en el presente contrato por parte del Locatario, y especialmente la demora en '+(L?'TREINTA DIAS':'treinta dias')+' o mas en el pago del alquiler, '+(L?'servicios, Gastos Comúnes si correspondiera,':C?'impuestos, servicios, expensas,':D?'servicios, expensas,':'servicios,')+' intereses, ajustes, adicionales etc., facultara al Locador a declararlo rescindido, pudiendo exigir el desalojo inmediato como si se tratara de un contrato vencido. Sin perjuicio de reclamar las responsabilidades en que hubiese incurrido el Locatario y/o su '+(L?'personal':'familia')+', por los danos y perjuicios que erogue esta situacion.-');
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('Las partes manifiestan que este contrato de locación se rige estrictamente por las disposiciónes referidas al Contrato de Locación, artículo 1187 siguientes y concordantes del Código Civil y Comercial, dejándose aclarado que si al vencimiento del término contractual el Locatario continuase en el uso y ocupacion de la unidad alquilada, sin perjuicio de la accion de desalojo que corresponda al Locador y en concepto de cláusula punitiva e indemnización (art.790 CCYC), abonará ademas del importe del alquiler mensual último, una suma diaria de Pesos equivalentes al DIEZ POR CIENTO (10%) de la merced locativa vigente, por cada dia que demorase en la restitución del bien locado.-');
    // GARANTES
    c+=wP(cl(),{b:true,sb:200,sa:80});
    var garTxt='';
    ...
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('A los efectos de la restitución del inmueble una vez vencido el término contractual, se aclara que el Locatario deberá proceder de la siguiente manera: 1) El inmueble deberá entregarse en el mismo buen estado de conservación e higiene que fue entregado según consta en el presente contrato'+(SE?', salvo el desgaste natural producido en las cosas por su correcto uso y el transcurso del tiempo. Ademas deberá presentar':', debiendo presentar')+' los comprobantes que demuestren los servicios de mantenimiento realizados a: '+rest+'. 2) '+bajaTxt);
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('Las partes acuerdan que durante todo el periodo en que el Locatario tenga la tenencia del inmueble locado y hasta la recepcion definitiva del mismo por parte de la Locadora y/o su representante, esta queda eximida de cualquier tipo de responsabilidad contractual o extracontractual por danos de cualquier especie que sufran personas y/o cosas en el inmueble locado y/o sus dependencias. A tal efecto el Locatario '+(C?'contratará':'deberá contratar')+' un seguro de responsabilidad civil hacia terceros por cualquier tipo de hechos que ocurrieran en el inmueble locado durante la vigencia del presente y aun vencido el mismo hasta tanto el Locatario haga devolución efectiva del inmueble al Locador.-');
    c+=wP(cl(),{b:true,sb:200,sa:80});
    var rescT='El Locatario podrá rescindir el presente contrato despues de los primeros cuatro meses de arriendo, debiendo comúnicar en forma fehaciente y con, al menos, sesenta (60) dias de anticipacion al LOCADOR. De producirse tal alternativa durante el primer ano de alquiler, el LOCATARIO deberá abonar el equivalente a un mes y medio de alquiler, en concepto de indemnización, y el equivalente a un mes si rescindiera transcurrido un ano de contrato. De hacer uso de la opcion de resolución anticipada de contrato, el Locatario se obliga a facilitar al Locador dentro de los treinta dias habiles anteriores a la entrega del inmueble, su ofrecimiento y muestra a terceros interesados en ocuparlo, en dias habiles y horarios razonables, mínimamente una vez por semana.-';
    if(inclV) rescT+=' Asimismo el Locador podrá rescindir el presente contrato ante la posibilidad cierta de venta del inmueble objeto del presente; a tal efecto deberá notificar al Locatario con una antelacion de tres (3) meses a la fecha en que se solicite la desocupacion y poniendo a disposición del Locatario el instrumento que fundamente el pedido de rescisión (boleto de compraventa, seña, escritura, etc), no pudiendo ejercer dicha facultad de rescisión antes de que hayan transcurrido seis (6) meses desde la efectiva entrega y toma de posesion del inmueble por parte del Locatario.-';
    c+=wP(rescT);
    c+=wP(cl(),{b:true,sb:200,sa:80});
    ...
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('Se deja expresa constancia que '+g1G.el+'/la '+g1G.sr+'/a. '+(v(g1.id+'-n')||'XXXXXXXXXXXX')+', '+(g1.gen==='m'?'argentino':'argentina')+', mayor de edad, '+g1G.nacido+' el '+fL(v(g1.id+'-fn'))+', M.I. '+(v(g1.id+'-d')||'XXXXXXXX')+', con domicilio en '+(v(g1.id+'-dom')||'XXXXXX')+', de la ciudad de '+(v(g1.id+'-ciudad')||'XXXXXXXXX')+' en la provincia de Cordoba, garante de este contrato'+(cony1?' y su conyuge '+g1G.el+'/la '+g1G.sr+'/a. '+cony1+', MI. '+(v(g1.id+'-conyd')||'XXXXXXXX')+',':' y su conyuge la/el Sra/Sr, XXXXXXXXXXXXXX, MI. XXXXXXXX,')+' renuncia/n a los beneficios que le otorga la Ley Provincial 8067 (Inembargabilidad de la vivienda unica) en un todo de acuerdo a lo dispuesto por el Art. 4 de la mencionada Ley.- OJO QUE SIEMPRE DEBE FIRMAR EL CONYUGE SI ESTA CASADO PARA PRESTAR EL CONSENTIMIENTO',{col:colC});
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('Por el presente el Locador otorga mandato expreso a ROBERTS OTERO SERVICIOS INMOBILIARIOS S.A.S'+(D||C?',':'')+' para que en su nombre y representación, perciba los alquileres mensuales, otorgue el correspondiente recibo, formule las intimaciones previstas por el C.C.yC. y accione judicialmente persiguiendo el cobro y/o el desalojo del inmueble objeto de la locación si correspondiere y la realizacion de todo otro acto que estime necesario para una mejor administración del inmueble y mejora de la renta que el mismo puede proporciónar, quedando el locatario notificado de ello en forma fehaciente por la mera suscripcion de este contrato.-');
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('El sellado, honorarios y demas gastos que se originen en el presente contrato, serán abonados exclusivamente por el Locatario.-');
    c+=wP(cl(),{b:true,sb:200,sa:80});
    c+=wPcl('Las partes que suscriben este contrato renuncian al fuero federal o a cualquier otro de excepcion que pudiese corresponderles, y se someten a la jurisdicción de los Tribunales ordinarios de la ciudad de Cordoba para cualquier cuestión que por el presente pudiese suscitarse entre las mismas. A todos los efectos del presente contrato, las partes constituyen los siguientes domicilios especiales: La parte locadora en: la empresa administradora, ROBERTS OTERO SERVICIOS INMOBILIARIOS S.A.S, Avenida Rio de Janeiro N 1.725 Torre 1 oficina 11; e-mail secretariava@juarezbeltran.com.ar; la parte Locataria: en calle '+(v('loc2-dom')||'XXXXXX')+', email '+(v('loc2-em')||'XXXX')+', y los Fiadores constituyen domicilio especial en '+(v(g1.id+'-dom')||'XXXXXXXX')+', e-mail '+(v('loc-em')||'XXXXXXXX')+'. Conforme las partes previa lectura y ratificacion, se firman tres ejemplares de un mismo tenor y a un solo efecto, en la Ciudad de Villa Allende a '+fFirma(v('firma'))+'.-');
    ...
    c+=wL();
    // Apply any user clause edits (override generated text per clause)
    // CLAUSE_EDITS is applied via wCl() during generation - already handled
